...
logging.basicConfig(format=_fmt, level=logging.DEBUG)
...
def create_poll():
    try:
        ...
        conn.commit()
        cursor.close()
        logging.info('create success (vid=%s)'%vid)
    except Exception,e:
        logging.error('create error (msg=%s)'%str(e))
        return jsonify({"return_code":21, "return_msg":str(e), "p_id":0})
    return jsonify({"p_id":vid})

@app.route('/poll', methods=['POST','GET'])
def do_poll():
    if "p_id" in request.args: # show options when load this page
    ...
    o_id = int(request.form['opt_idx'])-1 # do poll
    ...
    try:
        ...
        conn.commit()
        cursor.close()
        logging.info('do_poll success (vid=%s)'%p_id)
    except Exception,e:
        cursor.close()
        logging.error('do_poll error (msg=%s)'%str(e))
...
def show_poll():
    ...
    try:
        ...
        logging.info('show_poll success (vid=%s)'%p_id)
        return render_template("show.html", title=title, opts=rows)
    except Exception,e:
        logging.error('show_poll error (msg=%s)'%str(e))
        return render_template("show.html", title=title)
...
def show_refresh():
    ...
    try:
        ...
        logging.info('refresh success (vid=%s)'%p_id)
        return jsonify({"result_code":"0", "result_msg":"success", "rows":rows})
    except Exception,e:
        logging.error('refresh error (msg=%s)'%str(e))
        return jsonify({"result_code":"-1", "result_msg":"refresh error"})
